...
public class EmployeeDAOImplMySql implements IEmployeeDAO {
	...
	private static String GET_EMPS_BY_DESG="SELECT EMPNO,ENAME,EJOB,ESALARY FROM EMP WHERE EJOB IN(?,?,?)";
	private static final String INSERT_EMP="INSERT INTO EMP(ENAME,EJOB,ESALARY) VALUES(?,?,?)";  
	@Autowired
	...
	int	result=0;
		try(Connection con=ds.getConnection();
			...
			{
				System.out.println("SQL Insert problem");
